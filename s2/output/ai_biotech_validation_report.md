# AI—创新药风格验证日报

## 1. 数据日期

- 报告日期：2026-06-18
- A股数据日期：20260618
- 港股数据日期：20260618
- 对应美股收盘日期：not_applicable
- AI_CORE版本：CN_AI_CORE_V1；China A-share technology proxy
- 有效样本数：211；历史上限：250

## 2. 今日状态

- AI状态：AI_3D_PLUS_UP
- 市场状态：RISK_ON
- 创新药相对医疗：0.16%
- 创新药相对AI：-2.83%
- 589720情绪状态：589720强于A股医疗宽基
- A股是否领先港股：589720_2d_leads_159567
- 右侧确认评分：54.70；初步右侧
- 评分置信度：medium

## 3. 多窗口结论

| 窗口 | AI—创新药关系 | 绝对上涨胜率 | 相对医疗跑赢胜率 | 稳定性 |
| --- | --- | ---: | ---: | --- |
| 20日 | 跑输AI | 0.25000000 | 0.45000000 | 未获得稳定验证 |
| 60日 | 跑输AI | 0.33333333 | 0.51666667 | 存在统计信号，需结合经济意义 |
| 120日 | 跑输AI | 0.39166667 | 0.49166667 | 存在统计信号，需结合经济意义 |
| 250日 | 跑输AI | 0.41706161 | 0.46445498 | 存在统计信号，需结合经济意义 |

## 4. 最强支持证据

- 159567当日跑赢159557 0.16%
- 159567当日绝对上涨 1.19%
- S1-05广度达到46.67%

## 5. 最强反对证据

- AI上涨时159567跑输AI -2.83%
- S2_conversion_score=0.50，交易转化未确认
- S1-06龙头先行强度为负 -0.62%

## 6. AI回调验证

- AI是否回调：否
- AI回调类型：AI_3D_PLUS_UP
- 市场是否Risk Off：否
- 159567绝对收益：1.19%
- 159567相对159557超额：0.16%
- 159567相对AI超额：-2.83%
- 是否属于有效轮动：当前AI未回调，不适用。

## 7. A股领先港股验证

- 589720是否先动：589720_2d_leads_159567
- S1是否先动：S1-04_upcross_1.00x，样本10，胜率0.80000000
- 可能领先窗口：1日
- 历史样本数：6
- 历史胜率：1.00000000
- 当前是否有效：A股与159567同步偏强，但不等于A股领先。

## 8. 当前命题

- AI资金虹吸假设：strengthened：AI上涨且159567跑输AI。
- 创新药接力假设：unchanged：仍需确认。
- A股领先港股假设：weakened：589720及S1对159567未表现出稳定领先价值，只适合作为同步情绪温度计。
- 状态：weakened

## 9. 仓位建议

- 建议：hold
- 理由：right_side_score=54.70; confidence=medium; thesis_state=weakened
- 下一增加条件：右侧确认评分>=70、置信度不低、159567相对159557和量能同时确认、反对证据少于2条。
- 下一减少条件：AI回调后159567仍不涨或跑输159557，或右侧评分<30，或S2_conversion继续低于0.60。
- 当前最强反对证据：AI上涨时159567跑输AI -2.83% | S2_conversion_score=0.50，交易转化未确认 | S1-06龙头先行强度为负 -0.62%

## 10. 当前仍不能确认的事项

- 美股AI_CORE未启用，当前无法验证美股AI对亚洲创新药的跨市场虹吸。
- 当前复权口径为unadjusted_close，ETF分红/拆分仍可能影响历史收益精度。
- 120/250日窗口包含重叠持有期统计，不等于独立交易次数。
- 若20/60日与120/250日结论冲突，应解释为近期关系变化，但长期稳定性不足。
- 本模块输出仓位动作标签，不构成投资建议。
