...
注意：每日更新不只是跑代码生成报告，还包括 `s2/agent_task.md` 定义的事件智能体扫描。即使没有新增事件，也必须留下 `s2/output/agent_runs/YYYY-MM-DD.md` 运行记录，说明检查范围和无新增原因。
...
不要只跑 `s2.generate_s2_report` 或只跑 `s2.build_data_layer` 来更新日报。
两者都不是完整日报链路。

如果必须手动分步排查，顺序如下：

```bash
# 1. 更新 S1 原始数据、S1 指标和 S1 日报
./.venv/bin/python -m wb.daily_flow
...
重建 audit。当天更新日报前，必须确认：

- `data/indicators/YYYYMMDD.json` 已更新到最新 S1 交易日。
- `s2/output/hk_cache/159567.csv` 与 `s2/output/hk_cache/159557.csv`
  已刷新，且报告中披露 `latest_date_159567/latest_date_159557`。
- `data/processed/market_daily.csv` 已重建。
- `s2/output/data_audit/market_data_audit.csv` 已重建。
- `s2/output/reports/YYYY-MM-DD.md` 的报告日期等于最新 S1 交易日。

### 为什么不能跳过 `s2.update_market_data`

S2 目前存在两套 ETF 行情使用场景：

| 场景 | 读取文件 | 用途 |
| --- | --- | --- |
| S2/AI 数据层 | `data/processed/market_daily.csv` | S2-04、AI/科技成长、audit、回测 |
| HK_observation | `s2/output/hk_cache/159567.csv`、`s2/output/hk_cache/159557.csv` | 判断 159567 是否强于 159557 |

`s2.build_data_layer` 会更新 `market_daily.csv`，但不会替代
`s2.update_market_data` 对 `hk_cache` 的刷新。若漏跑 `s2.update_market_data`，
可能出现：

```text
market_daily 已经到 20260622
hk_cache 仍停在 20260618
S2 报告把 HK_observation 写成 valid_common_date 或 stale
```

因此日报更新必须先刷新 HK_observation 缓存，再生成 S2 报告。
...
- S2 报告中的 “S1更新状态” 应显示同一个最新日期。

`s2/output/s1_update_status.json` 是历史展示元数据，不再作为 S2 报告的唯一
状态来源。S2 报告会直接读取：

```text
data/indicators/*.json
data/raw/fund_daily.csv
data/raw/fund_share.csv
```

来生成 S1 更新状态。若 `s1_update_status.json` 滞后，不得据此判断 S1 没有
更新。
...
该文件不是 S1 原始计算结果。若它滞后，只能作为备注，不能据此改 S1 指标值。
